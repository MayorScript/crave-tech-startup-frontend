...
function Phases(props) {
	...
	function addListHandler(e) {
		...
		const phase = { id: uuid(), title: newListTitleRef.current.value, completed: false };
		props.onAddList(phase);
	...
	return (
		<div className="flex flex-col flex-grow overflow-auto p-6">
			...
			{
				props.data.map((phase) => (
				<NavLink
					key={phase.id}
					to={"/phase/" + phase.id}
					className={`${phase.completed ? "pointer-events-none" : ""} flex w-full font-medium px-4 py-3 mb-1 rounded-lg text-black text-opacity-70 bg-black bg-opacity-0 hover:bg-opacity-5 hover:text-opacity-100 dark:bg-white dark:text-white dark:bg-opacity-0 dark:text-opacity-70 dark:hover:bg-opacity-10 dark:hover:text-opacity-100`}
					activeClassName="bg-blue-500 text-blue-800 bg-opacity-10 text-opacity-90 hover:bg-opacity-20 dark:bg-blue-500 dark:bg-opacity-20 dark:text-blue-500 dark:text-opacity-90 dark:hover:bg-opacity-30 "
				>
					<ViewListIcon className="h-6 w-6 mr-3" />
					<div className="text-left flex-grow">{phase.title}</div>
				</NavLink>
			))}
